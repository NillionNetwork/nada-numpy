...
async def main():
    ...
    values_0 = await client.store_values(
        my_input_0, ttl_days=5, permissions=permissions
    ).invoke()
    ...
    output = na_client.float_from_rational(result["my_output"].value)
    print(f"✅  Compute complete for compute_id {compute_id}")
    print(f"🖥️  The result is {output}")
    return result
