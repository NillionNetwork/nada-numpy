...
def build_nada(test_dir):
    result = subprocess.run(["nada", "build"], cwd=test_dir, capture_output=True, text=True)
    if result.returncode != 0:
        pytest.fail(f"Build failed: {result.stderr}")

def run_nada(test_dir):
    result = subprocess.run(["nada", "test"], cwd=test_dir, capture_output=True, text=True)
    if result.returncode != 0:
...
def test_client():
    ...
    public_variables = nillion.PublicVariables(
        na_client.concat([
            ...
            na_client.array(np.zeros((3,3)), "D", nillion.PublicVariableUnsignedInteger)
        ])
    )
